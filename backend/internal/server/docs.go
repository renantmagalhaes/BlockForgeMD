...
var CATS=[
  ...
  {id:'plugins',label:'Plugins',items:[
    ...
    {method:'GET',path:'/api/plugins/google-calendar/config',sum:'Get shared Google OAuth config',
     desc:'Returns the instance-wide Google OAuth Client ID (plain), whether a Client Secret has been saved (the secret itself is never returned), the background sync interval, the workspace sync scope, whether the Testing-vs-Production reminder banner has been dismissed, plus the exact redirectUri to register in Google Cloud Console. All of this is shared instance-wide — the same for every user, not per-account.',
    ...
    {method:'POST',path:'/api/plugins/google-calendar/config',sum:'Set shared Google OAuth config',
     desc:'Saves the instance-wide Client ID/Secret (from a Google Cloud Console OAuth Client you create yourself), the background sync interval, the workspace sync scope, and/or the Production-confirmed acknowledgment — affects every user. Omit clientSecret to update just the Client ID and leave the previously saved secret untouched. Omit pollIntervalSeconds or productionConfirmed to leave them unchanged; minimum poll interval is 30 seconds. workspaces is always applied as given: an empty array (or omitted) means all workspaces sync; a non-empty array restricts sync to just those workspace names, best-effort deleting already-synced events (for every connected user) that fall outside the new scope. productionConfirmed is purely a manual UI acknowledgment (there is no API to verify actual OAuth publishing status) — setting it true just hides the reminder banner.',
