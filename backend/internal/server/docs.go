...
var CATS=[
  ...
  {id:'plugin-ollama-tagger',label:'Plugin · Ollama Auto Tags',items:[
    {method:'GET',path:'/api/plugins/ollama-tagger/config',sum:'Get personal Ollama auto-tag settings',
     desc:'Returns the current user\'s Ollama endpoint, model, schedule, change-recheck preference and plugin-managed tag limit.',
     curl:function(b){return b+'/api/plugins/ollama-tagger/config'}},
    {method:'POST',path:'/api/plugins/ollama-tagger/config',sum:'Save personal Ollama auto-tag settings',
     desc:'Saves personal Ollama configuration. The endpoint is encrypted at rest; scheduled and change-triggered tagging only use this user\'s configuration.',
     body:'{"endpoint":"http://ollama.example:11434","model":"llama3.2","autoEnabled":true,"recheckOnChange":true,"pollIntervalSeconds":900,"maxTags":5,"workspaces":["Default"]}',
     curl:function(b){return b+'/api/plugins/ollama-tagger/config'}},
    {method:'GET',path:'/api/plugins/ollama-tagger/models',sum:'List installed Ollama models',
     desc:'Verifies the personal Ollama endpoint and lists its installed models for the settings picker. Pass endpoint to verify an unsaved endpoint value.',
     curl:function(b){return b+'/api/plugins/ollama-tagger/models?endpoint=http%3A%2F%2Follama.example%3A11434'}},
    {method:'POST',path:'/api/plugins/ollama-tagger/tag-file',sum:'Generate tags for one Markdown file',
     desc:'Immediately classifies the specified open Markdown file with the current user\'s Ollama configuration.',
