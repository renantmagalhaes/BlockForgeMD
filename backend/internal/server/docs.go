...
var CATS=[
  ...
  {id:'plugins',label:'Plugins',items:[
    ...
    {method:'GET',path:'/api/plugins/google-calendar/config',sum:'Get shared Google OAuth config',
     desc:'Returns the instance-wide Google OAuth Client ID (plain) and whether a Client Secret has been saved (the secret itself is never returned), plus the exact redirectUri to register in Google Cloud Console.',
     curl:function(b){return b+'/api/plugins/google-calendar/config'},
     resp:'{"clientId":"123-abc.apps.googleusercontent.com","hasClientSecret":true,"redirectUri":"https://notes.example.com/api/plugins/google-calendar/oauth/callback"}'},
    {method:'POST',path:'/api/plugins/google-calendar/config',sum:'Set shared Google OAuth config',
     desc:'Saves the instance-wide Client ID/Secret (from a Google Cloud Console OAuth Client you create yourself). Omit clientSecret to update just the Client ID and leave the previously saved secret untouched.',
     body:'{"clientId":"123-abc.apps.googleusercontent.com","clientSecret":"GOCSPX-..."}',
     curl:function(b){return b+'/api/plugins/google-calendar/config'}},
