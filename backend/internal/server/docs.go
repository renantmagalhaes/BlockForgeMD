...
var CATS=[
  ...
  {id:'plugin-ollama-tagger',label:'Plugin · AI Auto Tags',items:[
	{method:'GET',path:'/api/plugins/ollama-tagger/config',sum:'Get personal AI auto-tag settings',
     desc:'Returns the current user\'s selected provider, model, schedule, change-recheck preference and plugin-managed tag limit.',
     curl:function(b){return b+'/api/plugins/ollama-tagger/config'}},
	{method:'POST',path:'/api/plugins/ollama-tagger/config',sum:'Save personal AI auto-tag settings',
     desc:'Saves personal provider configuration. Ollama endpoints and OpenRouter API keys are encrypted at rest; scheduled and change-triggered tagging only use this user\'s configuration.',
     body:'{"provider":"ollama","endpoint":"http://ollama.example:11434","model":"llama3.2","autoEnabled":true,"recheckOnChange":true,"pollIntervalSeconds":900,"maxTags":5,"workspaces":["Default"]}',
     curl:function(b){return b+'/api/plugins/ollama-tagger/config'}},
	{method:'GET',path:'/api/plugins/ollama-tagger/models',sum:'List models from Ollama or OpenRouter',
     desc:'Lists the selected provider\'s models for the settings picker. Pass an endpoint to verify an unsaved Ollama endpoint.',
     curl:function(b){return b+'/api/plugins/ollama-tagger/models?provider=ollama&endpoint=http%3A%2F%2Follama.example%3A11434'}},
    {method:'POST',path:'/api/plugins/ollama-tagger/tag-file',sum:'Generate tags for one Markdown file',
     desc:'Immediately classifies the specified open Markdown file with the current user\'s selected provider.',
     body:'{"path":"Default/Notes/example.md"}',
     curl:function(b){return b+'/api/plugins/ollama-tagger/tag-file'},
     resp:'{"status":"ok"}'},
  ]},
  {id:'plugin-google-calendar',label:'Plugin · Google Calendar',items:[
    {method:'GET',path:'/api/plugins/google-calendar/config',sum:'Get your own Google OAuth config',
     desc:'Returns the current user\'s own Google OAuth Client ID (plain), whether a Client Secret has been saved (the secret itself is never returned), their own background sync interval, their own workspace sync scope, whether they\'ve dismissed the Testing-vs-Production reminder banner, plus the exact redirectUri to register in Google Cloud Console. All of this is per-user — every user has their own Google OAuth Client and settings, none of it shared.',
     curl:function(b){return b+'/api/plugins/google-calendar/config'},
     resp:'{"clientId":"123-abc.apps.googleusercontent.com","hasClientSecret":true,"pollIntervalSeconds":120,"workspaces":[],"productionConfirmed":false,"redirectUri":"https://notes.example.com/api/plugins/google-calendar/oauth/callback"}'},
    {method:'POST',path:'/api/plugins/google-calendar/config',sum:'Set your own Google OAuth config',
     desc:'Saves the current user\'s own Client ID/Secret (from a Google Cloud Console OAuth Client they create themselves), background sync interval, workspace sync scope, and/or Production-confirmed acknowledgment — affects only this user\'s own sync. Omit clientSecret to update just the Client ID and leave the previously saved secret untouched. Omit pollIntervalSeconds or productionConfirmed to leave them unchanged; minimum poll interval is 30 seconds. workspaces is always applied as given: an empty array (or omitted) means all workspaces sync; a non-empty array restricts sync to just those workspace names, best-effort deleting this user\'s already-synced events that fall outside the new scope. productionConfirmed is purely a manual UI acknowledgment (there is no API to verify actual OAuth publishing status) — setting it true just hides the reminder banner.',
     body:'{"clientId":"123-abc.apps.googleusercontent.com","clientSecret":"GOCSPX-...","pollIntervalSeconds":120,"workspaces":["Default"],"productionConfirmed":true}',
     curl:function(b){return b+'/api/plugins/google-calendar/config'}},
