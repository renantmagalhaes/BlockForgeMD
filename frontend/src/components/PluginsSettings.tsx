...
export default function PluginsSettings() {
  ...
  const [view, setView] = useState<'grid' | 'google-calendar' | 'ai-auto-tags'>('grid')
  ...
  if (view === 'ai-auto-tags') return <AITagsDetail onBack={() => setView('grid')} />

  const googleCalendar = plugins.find(p => p.id === 'google-calendar')
  const aiAutoTags = plugins.find(p => p.id === 'ai-auto-tags')
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-150">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {aiAutoTags && <button onClick={() => setView('ai-auto-tags')} className="text-left bg-slate-900/50 hover:bg-slate-900 border border-slate-800 hover:border-violet-500/40 rounded-xl p-4 space-y-2 transition cursor-pointer"><div className="flex items-center gap-2"><Tags size={16} className="text-violet-400" /><span className="text-sm font-semibold text-slate-100">{aiAutoTags.name}</span></div><p className="text-[11px] text-slate-500">Use Ollama or OpenRouter to add and refresh contextual page tags.</p></button>}
...
type AITagsConfig = { provider: 'ollama' | 'openrouter'; endpoint: string; apiKey: string; hasApiKey?: boolean; model: string; autoEnabled: boolean; recheckOnChange: boolean; pollIntervalSeconds: number; maxTags: number; workspaces: string[] }
function AITagsDetail({ onBack }: { onBack: () => void }) {
  const [c,setC]=useState<AITagsConfig>({provider:'ollama',endpoint:'http://10.0.10.11:11434',apiKey:'',model:'',autoEnabled:false,recheckOnChange:true,pollIntervalSeconds:900,maxTags:5,workspaces:[]}); const [msg,setMsg]=useState(''); const [saving,setSaving]=useState(false)
